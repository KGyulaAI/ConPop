namespace ConPop
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            CityDataProvider cityData = new CityDataProvider();
            cityData.LoadFromCSV("pop_city.csv");

            //3
            Console.WriteLine(cityData.CityCount());

            //4
            foreach (City city in cityData.Top10City(2020))
            {
            ...
            while (string.IsNullOrEmpty(inputCity))
            {
                Console.WriteLine("Írd be egy város nevét:");
                inputCity = Console.ReadLine();
                City foundCity = cityData.Cities.First(x => x.CityName.ToLower() == inputCity.ToLower());

                if (cityData.Cities.Any(x => x.CityName.ToLower() == inputCity.ToLower()))
                {
                    List<int> populationDatas = new List<int>();
                }
            }
        }
    }
